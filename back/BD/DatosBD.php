...
if (!isset( $_GET['folio'])) { //general data
	//echo $_POST['folio'];
//declarar consulta 
$sqlGeneral= 
...
} else { //if there is a specific id
	$folio = $_GET['folio'];
	$sqlSpecific =
		"SELECT
			*
		FROM
			datos_generales
		JOIN 
			datos_niño ON datos_niño.Folio = datos_generales.Folio
		JOIN 
			datos_derecho ON datos_derecho.Folio = datos_generales.Folio
		JOIN 
			conyuge ON conyuge.Folio = datos_generales.Folio
		WHERE
			datos_generales.Folio = '$folio'";
	
	$respGen=mysqli_query($conexion,$sqlSpecific);//hacer consulta
	$datosGen=mysqli_fetch_assoc($respGen);//guardar resultado
	$resultado=json_encode($datosGen); //convertir a formato json
}
